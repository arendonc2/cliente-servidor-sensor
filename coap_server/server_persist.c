// server_persist.c - Servidor CoAP simple con persistencia a archivos (C, sockets UDP)
// Endpoints:
...
//
// Compilar:   gcc -std=c99 -O2 -Wall -Wextra -o server_persist server_persist.c
// Ejecutar:   ./server_persist
// Variables de entorno (opcional):
//   COAP_DATAFILE    (default: "/opt/coap/data.txt")
//   COAP_CURRENTFILE (default: "/opt/coap/current.txt")
//
// Recomendado en EC2 (evita permisos en /opt):
//   mkdir -p /home/ec2-user/coap_data
//   export COAP_DATAFILE=/home/ec2-user/coap_data/history.log
//   export COAP_CURRENTFILE=/home/ec2-user/coap_data/current.state
//   ./server_persist &
...
static void extract_state(const char* line, char* out, size_t outsz){
    ...
    if (p) strncpy(out, p + strlen(tag), outsz-1);
    else   strncpy(out, line, outsz-1);
    out[outsz-1] = '\0';
}
...
static int coap_parse(const uint8_t* buf, size_t len, coap_req_t* r){
    ...
    while (p < end && *p != 0xFF){
        ...
        if (num == OPT_URI_PATH){
            if (r->uri_path[0] && strlen(r->uri_path) < sizeof(r->uri_path)-1) strncat(r->uri_path, "/", sizeof(r->uri_path)-1);
            strncat(r->uri_path, (const char*)p, (size_t)l);
        }
        p += l; last = num;
    }
    if (p < end && *p == 0xFF){
    ...
    while (!g_stop){
        ...
        if (req.code == COAP_GET && strcmp(req.uri_path, "sensor") == 0){
            ...
            if (read_last_line(CURR, line, sizeof(line))){
                strncpy(resp, line, sizeof(resp)-1); rlen = strlen(resp); rcode = COAP_205_CONTENT;
            } else if (read_last_line(DATA, line, sizeof(line))){
                char st[2048]; extract_state(line, st, sizeof(st));
                strncpy(resp, st, sizeof(resp)-1); rlen = strlen(resp); rcode = COAP_205_CONTENT;
            } else {
                strcpy(resp, "NO_DATA"); rlen = strlen(resp); rcode = COAP_205_CONTENT;
            }
        }
            ...
            if (write_current_and_history(CURR, DATA, body) == 0){
                ...
                strcpy(resp, "WRITE_FAIL"); rlen = strlen(resp); rcode = COAP_500_INTERR;
            }
        }
            ...
            strcpy(resp, "NOT_FOUND"); rlen = strlen(resp); rcode = COAP_404_NOTFOUND;
        }
