// server.c - Servidor CoAP mínimo (C puro) con sockets UDP + LOG a TXT.
// Endpoints:
//   POST /echo    -> responde "echo: <payload>" y LOG
//   PUT  /sensor  -> guarda estado en memoria y LOG
//   GET  /sensor  -> devuelve estado actual y LOG
//
...
/* ---- Utils: nombre método y LOG a archivo ---- */
static const char* method_name(uint8_t code) {
  switch (code) {
...
static void log_to_file(const char* ip, int port,
  ...
  fprintf(f, "%s %s:%d %s /%s | %s\n",
          ts, ip, port, method, (path && *path) ? path : "",
          (body && *body) ? body : "");
  fflush(f);
  fsync(fileno(f)); // asegurar escritura en disco
  fclose(f);
}
...
int main(void) {
  ...
  while (!stop_flag) {
    ...
    if (coap_parse(inbuf, (size_t)n, &req) != 0) {
      fprintf(stderr, "Paquete CoAP inválido de %s:%d\n", ipstr, cport);
      continue;
    }

    const char *path = req.uri_path[0] ? req.uri_path : "";
    const char *meth = method_name(req.code);

    // Copiamos payload como texto seguro
    ...
    if (strcmp(path, "echo") == 0 && req.code == COAP_POST) {
      // POST /echo
      ...
      // LOG al archivo
      log_to_file(ipstr, cport, "POST", path, body);

    } else if (strcmp(path, "sensor") == 0) {
      if (req.code == COAP_PUT) {
        // PUT /sensor -> guardar estado y log
        snprintf(g_state, sizeof(g_state), "%s", body);
        snprintf(resp, sizeof(resp), "UPDATED");
        rlen = strlen(resp);
        code = COAP_204_CHANGED;

        log_to_file(ipstr, cport, "PUT", path, body);

      } else if (req.code == COAP_GET) {
        // GET /sensor -> devolver el último estado y log
        snprintf(resp, sizeof(resp), "%s", g_state);
        rlen = strlen(resp);
        code = COAP_205_CONTENT;

        log_to_file(ipstr, cport, "GET", path, "");

      } else {
        code = COAP_405_METHODNA;
        snprintf(resp, sizeof(resp), "METHOD_NOT_ALLOWED");
        rlen = strlen(resp);

        log_to_file(ipstr, cport, meth, path, body);
      }
